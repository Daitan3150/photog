...
function AdminSidebar({ isCollapsed, toggleSidebar }: { isCollapsed: boolean; toggleSidebar: () => void }) {
    ...
    return (
        <aside
            className={`${sidebarBg} min-h-screen text-white fixed left-0 top-0 shadow-2xl z-50 transition-all duration-500 flex flex-col
        ...
        >
            {/* Header: Logo & Toggle */}
            <div className={`mb-10 pt-2 flex items-center justify-between relative`}>
                {!isCollapsed && (
            ...
            <nav className="space-y-2 flex-grow">
                ...
                <div className={`h-px ${isAdmin ? 'bg-slate-800' : 'bg-fuchsia-500/30'} my-4 mx-2`}></div>
            ...
            <div className={`mt-auto pb-6 ${isCollapsed ? 'w-full flex justify-center' : 'w-full'}`}>
                <button
            ...
            <div className="min-h-screen bg-gray-100 flex transition-all duration-300">
                ...
                <main
                    className={`flex-1 transition-all duration-300 ${!isPublicPage
                        ? (isCollapsed ? 'ml-20 p-4 md:p-8' : 'ml-64 p-4 md:p-8')
                        : ''
                        }`}
                >
                    {children}
